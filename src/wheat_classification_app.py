...
def load_config():
    ...
    try:
        ...
        st.error("config.yaml file not found. Please make sure it's in the same directory as this script.")
        return {}
        ...
        if st.button("Classify Wheat Kernel", key="single_predict"):
            with st.spinner("Classifying..."):
                ...
                if all_valid and model:
                    ...
                    # Make prediction
                    time.sleep(0.5)  
                    prediction, confidence, probabilities = predict(model, input_df)
    ...
    with tab2:
        ...
        **Required CSV format:**
        ...
        if uploaded_file is not None:
            ...
            batch_data = process_batch_csv(uploaded_file)
            ...
            if batch_data is not None and not batch_data.empty:
                ...
                if st.button("Run Batch Classification", key="batch_predict"):
                    if model:
                        ...
                        # Make predictions
                        predictions, confidence, probabilities = predict(model, batch_data)
